<?php
/**
 * Mori WP functions and definitions
 *
 ...
 */
function mori_wp_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on Mori WP, use a find and replace
		* to change 'mori-wp' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'mori-wp', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'mori-wp' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'mori_wp_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'mori_wp_setup' );
 ...
 */
function mori_wp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'mori_wp_content_width', 640 );
}
add_action( 'after_setup_theme', 'mori_wp_content_width', 0 );
 ...
 */
function mori_wp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'mori-wp' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'mori-wp' ),
...
add_action( 'widgets_init', 'mori_wp_widgets_init' );
...
add_action( 'wp_enqueue_scripts', 'mori_wp_scripts' );
